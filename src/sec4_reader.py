...
def read_sec4_to_dataframe(df, df2, ):
    for i, row in df.iterrows():
        ...
        derivativeTransactions = xml_root.findall(
            "./derivativeTable/derivativeTransaction")
        nonDerivativeTransactions = xml_root.findall(
        ...
        for t in allTransactions:
            ...
            tradingSymbol = read_tag(
                ticker_element, "./issuerTradingSymbol", '')

            # Owner Relationship
            owner = xml_root.findall("./reportingOwner")[0]
            rptOwnerName = read_tag(owner, './reportingOwnerId/rptOwnerName')
            isDirector = int(
                read_tag(owner, './reportingOwnerRelationship/isDirector', 0))
            isOfficer = int(
                read_tag(owner, './reportingOwnerRelationship/isOfficer', 0))
            isTenOwner = int(
                read_tag(owner, './reportingOwnerRelationship/isTenPercentOwner', 0))
            isOther = int(
                read_tag(owner, './reportingOwnerRelationship/isOther', 0))
            if isDirector == 1:
                owner = 'Director'
            elif isOfficer == 1:
                owner = 'Officer'
